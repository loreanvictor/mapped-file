...
describe(File, () => {
  ...
  test('.range() throws an error when out of range.', () => {
    const file = new File('file.ts', 'line1\nline2\nline3\nline4\nline5')
    ...
    }, { surrounding: 1 })).toThrow(RangeError)
  })
  ...
  test('.position() throws an error when out of range.', () => {
    const file = new File('file.ts', 'line1\nline2\nline3\nline4\nline5')

    expect(() => file.position(-1)).toThrow(RangeError)
    expect(() => file.position(30)).toThrow(RangeError)
  })
...
baz:
  ...
  test('.offset() throws an error when accessing out of range.', () => {
    const file = new File('file.ts', 'line1\nline2')

    expect(() => file.offset({ line: 2, character: 3 })).toThrow(RangeError)
  })
})
